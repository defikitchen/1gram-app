<template>
  <Page>
    <span class="clickable" @click="copy(tx.hash)" v-if="tx.hash">
      <PageSubtitle>{{ shortify(tx.hash) }}</PageSubtitle>
    </span>
      ...
      <v-list v-if="tx" class="list--clean">
        ...
        <v-list-item>
          <v-list-item-content>
            ...
            <v-list-item-title class="mt-2">
              {{ token(fees, wallet.network.symbol, wallet.network.decimals) }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <v-divider class="my-1" />
        ...
        <v-list-item>
          <v-list-item-content>
            ...
            <v-list-item-title class="mt-2">
              {{ time((tx.timestamp || 0) * 1000, "MM/DD/YYYY, hh:mm:ss") }}
              <span class="half-transparent"
                >({{ fromNow((tx.timestamp || 0) * 1000) }})</span
              >
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
