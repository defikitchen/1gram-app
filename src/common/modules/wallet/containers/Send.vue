...
import { useRoute, useRouter } from "@/common/hooks/use-router";

export default defineComponent({
  setup() {
    const {
      store: { state, commit, original, getters }
    } = useVuex();
    ...
    const getFees = async () => {
      ...
      try {
        ...
        if (wallet.value.network.protocol === "ton") {
          ...
          if (!pin)
            pin = await original.dispatch("Common/Login/promptPin", {
              persistent: false
            });
    ...
    const submit = () => {
      ...
      if (hasError.value) {
        ...
        original.commit("Wallet/setPendingTx", transaction);
        resetForm();
